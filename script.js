...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const parseMarkdown = (text) => {
        ...
        html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" style="color:var(--accent-color); text-decoration:underline; font-weight:600;">$1</a>'); // Links
        html = html.replace(/\n/g, '<br/>'); // Newlines
        return html;
    };
    ...
    const fetchGeminiResponse = async (query) => {
        ...
        // 1. Room Availability
        if (q.includes("room") && (q.includes("available") || q.includes("vacant") || q.includes("book"))) {
            const vacantRooms = MOCK_ROOMS.filter(r => !r.occupied);
            if (vacantRooms.length === 0) return "I'm sorry, but we are currently fully booked.";
            const roomsInfo = vacantRooms.map(r => `* **Room ${r.id}** (${r.type}): ฿${r.price}`).join("\n");
            return `We currently have the following rooms available:\n${roomsInfo}\n\nTo book, please contact our staff on LINE **@906vqhos**!`;
        }

        // 2. Breakfast Time
        if (q.includes("breakfast") || q.includes("food") || q.includes("eat")) {
            return "Our complimentary breakfast buffet is served daily from **6:30 AM to 10:00 AM** in the main lobby dining area. Enjoy your meal!";
        }

        // 3. Self Check-in / Check-out
        if (q.includes("check") && (q.includes("in") || q.includes("out") || q.includes("self"))) {
            return "For **Self Check-in**, please use the terminal on our website. Enter your ID or Passport number, and the system will instantly provide your Room Number and Digital Door Lock PIN to head straight to your room without seeing reception!\n\nFor **Check-out**, simply leave your keycard in the drop-box at the lobby.";
        }
